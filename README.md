# ⚽ Football Scoreboard

A customizable real-time football scoreboard system.

---

## 🚀 Current Features

### ✅ Completed
- Change team names and team colors  
- Score system  
- Match timer (including add times)  
- First half / Second half system  
- Goal animation  

---

### ⌛ In Progress
- Yellow / Red card system  
- Substitution display system  

---

### 🔜 Planned
- Highlight a selected team (focus mode)  
- Add additional logos beyond the main logo  
- Custom goal animations for specific teams  

---

### ❌ Not Planned
- None at the moment  

---

## 📌 Status Legend

- ✅ = Completed  
- ⌛ = In Progress  
- 🔜 = Planned  
- ❌ = Not Planned  

---

## How to use it

- 1.Open terminal on the folder
- or in Windows, go to the folder, right click and click "Open in Terminal"
- 2.Type "node server.js" in the terminal
- 3.To view the scoreboard, open your browser and type ``localhost:3000`` on the adressbar.
  <img width="1755" height="41" alt="image" src="https://github.com/user-attachments/assets/e1f60506-fc17-4517-91d2-f57c9f97c5d8" />

- 4.To change team names, colors, start the game, etc. Follow the step 3 but change ip to ``localhost:3000/controller``
  <img width="1739" height="41" alt="image" src="https://github.com/user-attachments/assets/8683a8eb-886d-4ae0-9b51-ee42edc10258" />


### How to use it for live streaming (OBS)

- 1.Add a source "Browser".
  <img width="1920" height="1074" alt="image" src="https://github.com/user-attachments/assets/7cc66f3f-44ec-47eb-b325-89c85778cc1b" />

- 2.Change the URL to ``localhost:3000`` and press OK.
  <img width="762" height="774" alt="image" src="https://github.com/user-attachments/assets/ca792638-4236-4b44-9360-4a39ec1b0c58" />

- 3.Adjust the size by your opinion.
  <img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/3bbdc219-11cb-42c2-8133-462e1a6d64bb" />

- (Optional) 4.If you're using studio mode, press "Transition" to make it displays in program scene.
  <img width="1919" height="660" alt="image" src="https://github.com/user-attachments/assets/132e0b0d-90f7-4bc9-9a04-106aac4bbaaf" />
